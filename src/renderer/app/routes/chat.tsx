...
function validateSceneSlots(
  ...
  for (const field of profile.fields) {
    ...
    if (field.type === "mindmap") {
      ...
      if (!node?.topic?.trim()) return `请填写${field.label}`
      continue
    }
    if (field.type === "flowchart") {
      const graph = v as { nodes?: unknown[] } | undefined
      if (!graph?.nodes?.length) return `请填写${field.label}`
      continue
    }
...
export default function ChatRoute() {
  ...
  const { data: activeScene } = useQuery({
    ...
    queryFn: async () => {
      ...
      return getScene(activePlaybookId!)
    },
    ...
    scenePlaybook != null
      ...
      : sceneProfile != null
        ? [
            SYSTEM_PROMPT,
            "",
            `【当前输入场景】${sceneProfile.name ?? sceneProfile.id}`,
            sceneProfile.description ?? "",
            "用户右侧面板中的参数为任务主输入；聊天框可写补充说明。",
          ].join("\n")
        : SYSTEM_PROMPT
            ...
            <Sheet open={optionsOpen} onOpenChange={setOptionsOpen}>
              ...
              <SheetContent side="right" className="w-full sm:max-w-md">
                <SheetHeader>
                  ...
                  <SheetDescription>模型参数与工具 trace</SheetDescription>
                </SheetHeader>
        ...
        <aside className="hidden w-80 shrink-0 flex-col border-l border-border/60 bg-card lg:flex">
          <div className="border-b border-border/60 px-4 py-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground line-clamp-2">
              {scenePlaybook?.description ?? sceneProfile?.description ?? ""}
            </p>
            {scenePlaybook && !scenePlaybook.builtin ? (
              <Link
                to={`/studio/playbook-designer?edit=${encodeURIComponent(scenePlaybook.id)}`}
